...
from estimation.process.models import Workstation, Operation, \
    Activity, ActivitySpeed, ActivityExpense
...
@pytest.fixture
def activity(db, activity_factory:Activity):
...
@pytest.fixture
def activity_offset_printing(db):
    speed = (120, 'pc', 'min')
...
def speed_factory(db):
    def create(measure_value, measure_unit, speed_unit, activity):
        return ActivitySpeed.objects.create(
...
def test_activity__get_duration_short(db, activity: Activity):
    ...
    assert activity.speed.rate.m__hr == 30
